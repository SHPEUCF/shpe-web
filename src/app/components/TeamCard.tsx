...
const TeamCard = ({ name, pos, pic, handleOpen, id }: TeamCardProps) => {
  return (<div className="2xl:w-80 2xl:h-[27rem] w-56 h-[22rem] p-2 pr-4 pt-4 bg-white rounded-lg shadow border border-stone-300 flex-col justify-between items-center gap-4 inline-flex">
    ...
    <button onClick={() => handleOpen(id)} className="2xl:w-60 2xl:h-12 2xl:text-xl mb-4 w-40 h-9 px-7 py-4 bg-yellow-400 hover:bg-yellow-500 rounded-md shadow justify-center items-center gap-2 inline-flex text-black text-lg font-medium">More</button>
  </div>
  )
}
